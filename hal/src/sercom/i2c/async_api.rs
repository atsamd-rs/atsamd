...
impl<S: Sercom> Handler<S::Interrupt> for InterruptHandler<S> {
    ...
    unsafe fn on_interrupt() {
        ...
        // Disable interrupts, but don't clear the flags. The future will take
        // care of clearing flags and re-enabling interrupts when woken.
...
mod dma {
        ...
        D: AnyChannel<Status = ReadyFuture>,
    {
        ...
        pub(super) async unsafe fn write_linked(
            ...
            // Unfortunately, gotta take a polling approach here as there is no
            // interrupt source that can notify us of an IDLE bus
            // state. Fortunately, it's usually not long. 8-10 times
            // around the loop will do the trick.
