...
use crate::dma::Error;
use crate::target_device::{dmac, DMAC, NVIC, Interrupt};
use core::ops::Deref;
...
pub(super) fn init_interrupts(nvic: &mut NVIC) {
    ...
    unsafe { nvic.set_priority(Interrupt::DMAC, (1 << NVIC_PRIO_BITS) - 1); }
}
...
pub(crate) struct ControllerCritical {
    ...
    channel_mask: VolatileCell<ChannelMask>,
}
...
impl ControllerCritical {
    ...
    pub fn read_channel_id_and_flags(&self, flags: u8) -> (u8, ChannelId) {
        (flags, 0)
    }
    ...
    pub fn set_priority(&self, channel_id: ChannelId, priority: super::Priority) {
        debug_assert_eq!(channel_id, 0);
        self.dmac.chctrlb.write(|reg| reg.lvl().bits(priority as u8));
    }
    ...
    /// Set action
    pub fn set_action(&self, channel_id: ChannelId, action: dmac::chctrlb::TRIGACTW) {
        unsafe {
            self.dmac.chid.write(|reg| reg.id().bits(channel_id));
    ...
    /// Set trigger
    pub fn set_trigger(&self, channel_id: ChannelId, trigger: dmac::chctrlb::TRIGSRCW) {
        unsafe {
            self.dmac.chid.write(|reg| reg.id().bits(channel_id));
            self.dmac
                .chctrlb
