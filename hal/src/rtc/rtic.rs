...
//! TODO: Written with the intention of no HAL dependence for move.
//! `rtic-monotonics`?
//!
//! # Example
//!
...
// TODO: Before HAL PR
// - Make sure every chip compiles
// - Complete documentation

mod v1 {
    use super::super::{Count32Mode, Rtc};
...
/// Create an RTIC v2 monotonic based on RTC in mode 0.
/// TODO: Where do we put more documentation? Maybe should be here.
/// For sure document the clock types.
#[macro_export]
...
// TODO: Given how this is done now, move these numbers to some documentation
// for some other item, a table for each macro
...
pub struct RtcMode0Backend;

static RTC_MODE0_TQ: TimerQueue<RtcMode0Backend> = TimerQueue::new();

#[hal_macro_helper]
impl RtcMode0Backend {
    #[inline]
    fn sync() {
    ...
    ///
    /// TODO: Update
    /// Use the [`rtc_mode0_monotonic`] macro instead.
    pub fn _start<S: rtc_clock::RtcClockSetter>(
        rtc: pac::Rtc,
        mclk: &mut pac::Mclk,
...
impl TimerQueueBackend for RtcMode0Backend {
    type Ticks = u32;
...
/// RTC based [`TimerQueueBackend`].
pub struct RtcMode1Backend;

static RTC_MODE1_PERIOD_COUNT: AtomicU64 = AtomicU64::new(0);
static RTC_MODE1_TQ: TimerQueue<RtcMode1Backend> = TimerQueue::new();

#[hal_macro_helper]
impl RtcMode1Backend {
    #[inline]
    fn sync() {
        let rtc = unsafe { &pac::Rtc::steal() };
    ...
    ///
    /// TODO: Update
    /// Use the [`rtc_mode1_monotonic`] macro instead.
    pub fn _start<S: rtc_clock::RtcClockSetter>(
        rtc: pac::Rtc,
        mclk: &mut pac::Mclk,
        osc32kctrl: &mut pac::Osc32kctrl,
        ...
        critical_section::with(|_| {
            ...
            RTC_MODE1_PERIOD_COUNT.store(0, Ordering::SeqCst);

            // Initialize the timer queue
            RTC_MODE1_TQ.initialize(Self {});
...
impl TimerQueueBackend for RtcMode1Backend {
    type Ticks = u64;

    #[hal_macro_helper]
    fn now() -> Self::Ticks {
        calculate_now(
            || RTC_MODE1_PERIOD_COUNT.load(Ordering::Relaxed),
    ...
    fn on_interrupt() {
        ...
        if intflag.ovf().bit_is_set() {
            ...
            let prev = RTC_MODE1_PERIOD_COUNT.fetch_add(1, Ordering::Relaxed);
