...
//! representing two different ways to handle GPIO pins. The default, [pin], is
//! a type-level API that tracks the state of each pin at compile-time. The
...
//! cost.
//!
//! If needed, [dynpin] can be used to erase the type-level differences between
//! pins. However, by doing so, pins must now be tracked at run-time, and each
//! pin has a non-zero memory footprint.
