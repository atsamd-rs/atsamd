...
[`UartTx`] both implement the DMAC [`Buffer`] trait. The provided 
[`send_with_dma`] and [`receive_with_dma`] build and begin a [`dmac::Transfer`],
...
use pac::sercom0::usart_int::ctrla::{MODE_A, RXPO_A, TXPO_A};
...
impl<S, I, RX, RTS> TxOrRx for Pads<S, I, RX, NoneT, RTS, NoneT>
where
    S: Sercom,
    I: IoSet,
    RX: SomePad,
...
impl<S, I, TX, CTS> TxOrRx for Pads<S, I, NoneT, TX, NoneT, CTS>
where
    S: Sercom,
    I: IoSet,
    TX: SomePad,
...
bitflags! {
    /// Status flags for UART Rx transactions
    ///
    /// The available status flags are `CTS`, `TXE` and `ITER`.
    /// The binary format of the underlying bits exactly matches
...
/// returns an enabled [`UartRx`], [`UartTx`], or both. The [`enable`] method is
    ...
    Self: Sized,
{
    ...
    fn disable(self) -> C {
        ...
        let usart = config.as_mut().sercom.usart_int();
