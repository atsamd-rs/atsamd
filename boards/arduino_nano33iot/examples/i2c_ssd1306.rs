...
const BOOT_DELAY_MS: u16 = 100; //small delay for the I2C to initiate correctly
                                // and start on boot without
...
fn main() -> ! {
    ...
    let mut buf = [0x00u8; 512]; //each line has 128 pixels, that's 8 bytes, x
                                 // 32 lines = 512 bytes for the
                                 // whole display

    let mut rng = SmallRng::seed_from_u64(0x0101_0303_0808_0909);

    loop {
        rng.fill_bytes(&mut buf); // repeatedly fills the display buffer with
