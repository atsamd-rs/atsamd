...
//!   switching
//!
//! 1. Sends a message over CAN on periodically and prints out transmit
...
mod app {
    ...
    fn init(mut ctx: init::Context) -> (Shared, Local, init::Monotonics) {
        ...
        let can1_rx = bsp::pin_alias!(pins.tcan1051_rx).into_mode();
        let can1_tx = bsp::pin_alias!(pins.tcan1051_tx).into_mode();

        bsp::pin_alias!(pins.tcan1051_s)
            .into_push_pull_output()
            .set_low()
            .ok();
        bsp::pin_alias!(pins.boost_en)
            .into_push_pull_output()
            .set_high()
            .ok();

        let (pclk_can1, gclk0) = clock::pclk::Pclk::enable(tokens.pclks.can1, clocks.gclk0);
        ...
        periodic_1hz::spawn_after(1.secs()).unwrap();

        let led = bsp::pin_alias!(pins.red_led).into();
        bump_activity_led();
    ...
    #[task(local = [counter: u16 = 0, tx_event_fifo, aux, tx])]
    fn periodic_1hz(ctx: periodic_1hz::Context) {
        bump_activity_led();
        rprintln!("Status:");
