...
#[embassy::main]
fn main(_spawner: embassy::executor::Spawner) -> ! {
    ...
    let mut xfer = Transfer::new_from_arrays(chan0, buf_src, buf_dest, false);
    xfer.begin_async(TriggerSource::DISABLE, TriggerAction::BLOCK)
        .await;

    // Grab resulting buffers
    let (chan0, buf_src, buf_dest) = xfer.free();
    ...
    let mut xfer = Transfer::new(chan0, const_16, buf_16, false).unwrap();
    xfer.begin_async(TriggerSource::DISABLE, TriggerAction::BLOCK)
        .await;

    let (chan0, const_16, buf_16) = xfer.free();
    ...
    // destination) with a 16-bit beat size
    let mut xfer = Transfer::new(chan0, buf_16, const_16, false).unwrap();
    xfer.begin_async(TriggerSource::DISABLE, TriggerAction::BLOCK)
        .await;

    let (chan0, buf_16, const_16) = xfer.free();
